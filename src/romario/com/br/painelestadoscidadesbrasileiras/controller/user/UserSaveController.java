 ...
 * @author Romario
 */
public class UserSaveController extends ApiBaseHelper {

    public ResponseInterface save(UserForm form) {
        try {
            AuthenticatedUser authenticatedUser = new AuthenticatedUser();

            ObjectMapper mapper = new ObjectMapper();

            String url = Constants.BASE_URL + Constants.URL_VERSION + "user/save";

            System.out.print(url + "\n");

            String jsonInString = mapper.writeValueAsString(form);

            Response response = getResponse(jsonInString, url, "POST", authenticatedUser.getToken());
